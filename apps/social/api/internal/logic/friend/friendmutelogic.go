package friend

import (
	"context"
	"fmt"

	"github.com/iceymoss/go-hichat-api/apps/social/api/internal/svc"
	"github.com/iceymoss/go-hichat-api/apps/social/api/internal/types"
	"github.com/iceymoss/go-hichat-api/apps/social/rpc/socialclient"
	"github.com/iceymoss/go-hichat-api/pkg/ctxdata"

	"github.com/zeromicro/go-zero/core/logx"
)

type FriendMuteLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewFriendMuteLogic(ctx context.Context, svcCtx *svc.ServiceContext) *FriendMuteLogic {
	return &FriendMuteLogic{
		Logger: logx.WithContext(ctx),
		ctx:    ctx,
		svcCtx: svcCtx,
	}
}

// 静音开关
func (l *FriendMuteLogic) FriendMute(req *types.FriendMuteReq) (*types.FriendMuteResp, error) {
	uid := ctxdata.GetUId(l.ctx)
	if uid == "" {
		return nil, fmt.Errorf("user id not found in context")
	}
	if req.FriendUid == "" {
		return nil, fmt.Errorf("friend_uid is required")
	}

	_, err := l.svcCtx.Social.FriendMute(l.ctx, &socialclient.FriendMuteReq{
		UserId:    uid,
		FriendUid: req.FriendUid,
		Muted:     req.Muted,
	})
	if err != nil {
		return nil, err
	}
	return &types.FriendMuteResp{}, nil
}
